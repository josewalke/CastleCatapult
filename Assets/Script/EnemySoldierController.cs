...
{
    public float speed = 5f;                // Velocidad a la que se mueve el soldado enemigo
    public string enemyTag = "Enemy";       // Tag para detectar enemigos (en este caso podría ser para evitar confusión entre enemigos)
    public string allyTag = "Ally";         // Tag para detectar aliados
    public string buildingTag = "Building"; // Tag para detectar edificios

    public int maxHealth = 100;             // Salud máxima del soldado enemigo
    public Image healthBar;                 // Imagen de la barra de salud en la UI
    public int attackDamage = 20;           // Daño que inflige el soldado enemigo al atacar
    public float attackCooldown = 1.5f;     // Tiempo de espera entre ataques
    private float lastAttackTime;           // Marca el tiempo en el que se realizó el último ataque

    public int currentHealth;               // Salud actual del soldado enemigo
    private bool isStopped = false;         // Estado que indica si el soldado está detenido
    private Collider currentTarget;         // Referencia al objetivo actual (soldado enemigo al que está atacando)
    private CastleController castleController; // Referencia al controlador del castillo, si se usa para el manejo de objetos
    ...
    void Update()
    {
        ...
        {
            Attack(); // Realiza el ataque si ha pasado el tiempo de enfriamiento
        }
    }
    ...
    void Attack()
    {
        ...
        {
            // Obtiene el componente SoldierController del objetivo actual
            SoldierController enemy = currentTarget.GetComponent<SoldierController>();
            if (enemy != null)
            {
    ...
    {
        // Reduce la salud del soldado enemigo en función del daño recibido
    ...
    void ReturnToPool()
    {
        // Desactiva el objeto del juego
        gameObject.SetActive(false);
        // Aquí podrías añadir más lógica si usas un sistema de pooling más complejo
    }
